...
class FlightHUD(Widget):
        ...
        for deg_raw in range(start, start + 101):
            ...
            if d % 10 == 0:
                ...
                if d % 30 == 0:
                    ...
                    tex = self._tex(label, h * 0.32, (1, 1, 1, 0.85))
    ...
    def _draw_speed_tape(self, x, y, w, h):
        ...
        for s in range(0, 50):
            ...
            if s % 5 == 0:
                ...
                tex = self._tex(str(s), w * 0.22, (1, 1, 1, 0.75))
        ...
        tex = self._tex(f"{spd:.1f}", bh * 0.6, (0, 1, 0, 1), bold=True)
        self._draw_tex(tex, x + (w - tex.width) / 2, cy - tex.height / 2)

        # Title label
        tex = self._tex("GS m/s", w * 0.18, (0.5, 0.55, 0.6, 1))
    ...
    def _draw_alt_tape(self, x, y, w, h):
        ...
        for a in range(-20, 200, 5):
            ...
            if a % 10 == 0:
                ...
                tex = self._tex(str(a), w * 0.22, (1, 1, 1, 0.75))
        ...
        Color(0, 1, 0, 1)
        Line(rectangle=(x, cy - bh / 2, w, bh), width=1)
        tex = self._tex(f"{alt:.1f}", bh * 0.6, (0, 1, 0, 1), bold=True)
        self._draw_tex(tex, x + (w - tex.width) / 2, cy - tex.height / 2)

        tex = self._tex("ALT m", w * 0.18, (0.5, 0.55, 0.6, 1))
        self._draw_tex(tex, x + (w - tex.width) / 2,
                       y + h - tex.height - 2)
    ...
    def _draw_info_bar(self, x, y, w, h):
        ...
        for i, text in enumerate(items):
            tex = self._tex(text, fs, (0.7, 0.8, 0.9, 1))
